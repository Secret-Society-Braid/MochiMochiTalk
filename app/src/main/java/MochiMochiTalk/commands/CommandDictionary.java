...
public class CommandDictionary extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@Nonnull SlashCommandInteractionEvent event) {
        if(event.getName().equals("dict")) {
            ...
            if(Objects.requireNonNull(phrase).equals("del")) { // since it requires phrase option, we don't need to check whether null or not
            ...
            try {
                ...
                event.replyFormat("辞書を更新中にエラーが発生しました。このことを report コマンドで報告する場合は以下のメッセージを開発者にお伝えください。%n%s", e.getMessage()).queue();
            }
        }
    }
    ...
    public Optional<String> getDict(String field) {
        ...
        try {
            ...
            return res.get("code").asInt() == 404 ? Optional.empty() : Optional.ofNullable(res.get("response").get("dict").asText());
        } catch (IOException e) {
