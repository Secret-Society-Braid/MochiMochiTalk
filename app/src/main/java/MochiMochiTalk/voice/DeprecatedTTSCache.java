...
public class DeprecatedTTSCache {
  ...
  private final ThreadPoolExecutor cacheService = new ThreadPoolExecutor(2, 4, 30, TimeUnit.SECONDS,
  ...
  DeprecatedTTSCache() throws Exception {
    ...
    if (!cacheFile.exists()) {
      ...
      cachedPhrases = load();
      cachedPhrases.forEach(
          (key, value) -> logger.info("cached sentences -> {} : {}", key, value.length));
    }
