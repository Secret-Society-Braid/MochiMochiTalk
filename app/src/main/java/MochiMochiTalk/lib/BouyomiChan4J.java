...
public class BouyomiChan4J {
    ...
    private final Logger logger = LoggerFactory.getLogger(BouyomiChan4J.class);
	...
	private void send(String host, int port, byte[] data) {
		...
		try {
			...
			logger.info("接続しました {}", socket.getRemoteSocketAddress());
			out = new DataOutputStream(socket.getOutputStream());
			out.write(data);
		} catch (ConnectException e) {
			logger.error("接続に失敗しました", e);
		} catch (IOException e) {
			logger.error("通信に失敗しました", e);
		} finally {
			try {
				if (out != null) {
					out.close();
