...
public class GoogleTTSEngine implements AudioSendHandler {
    ...
    byte[] tts(String phrase) throws IOException {
        ...
        try (TextToSpeechClient client = TextToSpeechClient.create(settings)) {
            ...
            AudioConfig config = AudioConfig.newBuilder().setAudioEncoding(AudioEncoding.LINEAR16).setSampleRateHertz(48_000).setVolumeGainDb(-2.0).build();
