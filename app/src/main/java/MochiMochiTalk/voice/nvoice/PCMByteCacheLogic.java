...
import MochiMochiTalk.voice.DeprecatedTTSCache;
...
public class PCMByteCacheLogic {
    ...
    CacheResponse checkCache(String phrase) {
        ...
        String base64Encoded = Base64.getEncoder().encodeToString(scrubed.getBytes());

        Optional<byte[]> optionalPcmByte = Optional.empty();
        try {
            optionalPcmByte = cacheController.getCacheBytes(base64Encoded);
        } catch (IOException e) {
